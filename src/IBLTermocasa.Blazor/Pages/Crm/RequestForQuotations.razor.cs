...
// Variabili per la gestione dei filtri
        ...
        private async void SearchAsync(string filterText)
        {
            ...
            if ((_searchString.IsNullOrEmpty() || _searchString.Length < 3) &&
                (RequestForQuotationMudDataGrid.Items != null && RequestForQuotationMudDataGrid.Items.Any()))
            {
                return;
            }
        ...
        private async Task<GridData<RequestForQuotationDto>> LoadGridData(GridState<RequestForQuotationDto> state)
        {
            ...
            var firstOrDefault = RequestForQuotationMudDataGrid.FilterDefinitions.FirstOrDefault(x =>
                x.Column is { PropertyName: nameof(RequestForQuotationDto.QuoteNumber) });
            if (firstOrDefault != null)
            {
                Filter.QuoteNumber = (string?)firstOrDefault.Value;
            }

            var firstOrDefault1 = RequestForQuotationMudDataGrid.FilterDefinitions.FirstOrDefault(x =>
                x.Column is { PropertyName: nameof(RequestForQuotationDto.OrganizationProperty) });
            if (firstOrDefault1 != null)
            {
                Filter.OrganizationProperty.Name = (string?)firstOrDefault1.Value;
            }

            var firstOrDefault2 = RequestForQuotationMudDataGrid.FilterDefinitions.FirstOrDefault(x =>
                x.Column is { PropertyName: nameof(RequestForQuotationDto.ContactProperty) });
            if (firstOrDefault2 != null)
            {
                Filter.ContactProperty.Name = (string)firstOrDefault2.Value!;
            }

            var firstOrDefault3 = RequestForQuotationMudDataGrid.FilterDefinitions.FirstOrDefault(x =>
                x.Column is { PropertyName: nameof(RequestForQuotationDto.AgentProperty) });
            if (firstOrDefault3 != null)
            {
                Filter.AgentProperty.Name = (string)firstOrDefault3.Value!;
            }

            var firstOrDefault4 = RequestForQuotationMudDataGrid.FilterDefinitions.FirstOrDefault(x =>
                x.Column is { PropertyName: nameof(RequestForQuotationDto.WorkSite) });
            if (firstOrDefault4 != null)
            {
                Filter.WorkSite = (string)firstOrDefault4.Value!;
            }

            var firstOrDefault5 = RequestForQuotationMudDataGrid.FilterDefinitions.FirstOrDefault(x =>
                x.Column is { PropertyName: nameof(RequestForQuotationDto.City) });
            if (firstOrDefault5 != null)
            {
                Filter.City = (string)firstOrDefault5.Value!;
            }

            var firstOrDefault6 = RequestForQuotationMudDataGrid.FilterDefinitions.FirstOrDefault(x =>
                x.Column is { PropertyName: nameof(RequestForQuotationDto.Status) });
        ...
        {
            if (input.Status is RfqStatus.DRAFT && !isReadOnly)
            {
...
// Metodi per la gestione dei filtri
        ...
        private async Task GetIdentityUserCollectionLookupAsync()
        {
            IdentityUsersCollection =
                (await RequestForQuotationsAppService.GetIdentityUserLookupAsync(new LookupRequestDto())).Items;
            AgentsList = IdentityUsersCollection.ToList();
        }

        private async Task GetContactCollectionLookupAsync()
        {
            ContactsCollection = (await RequestForQuotationsAppService.GetContactLookupAsync(new LookupRequestDto()))
                .Items;
